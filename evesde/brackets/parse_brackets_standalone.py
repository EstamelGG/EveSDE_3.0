...
def download_and_parse_brackets_files(eve_client=None):
    ...
    if eve_client is None:
        ...
        eve_client = get_eve_client(project_root / "cache/client")
...
def main(eve_client=None):
    ...
    try:
        ...
        # 写入 cache/brackets_output.json
        from evesde.paths import path as cfg_path, load_config
        try:
            output_file = str(cfg_path("brackets_output", load_config()))
        except Exception:
            output_file = str(PROJECT_ROOT / "cache" / "brackets_output.json")
